# Surviving Mutants — Phase 2 (Core UI)
...
### `src/lib/amendments.ts` — 7 survivors
...
**Decision**: The `parseValue` function is an internal display-formatting helper used only in the amendments log (read-only UI). No writes depend on it. The cost of adding exhaustively parametrised tests for every mutation path outweighs the risk, given the low business criticality of this formatting function.
