...
- [X] T041 Execute `specs/005-polish-hardening/quickstart.md` Part A (local validation of PIN, read-only, error states, backup banner) on the dev machine
- [X] T042 Update `docs/progress-log.md` — Phase 5 handoff: files created/modified, the calendar-month reconciliation decision + Constitution amendment, and the exact next step (operator runs quickstart Part B on the Pi)
- [ ] T043 **(BLOCKED — Pi-only, not executed)** Operator executes `specs/005-polish-hardening/quickstart.md` Part B (fresh-Pi E2E, FR-018) on the Pi and records completion in `docs/progress-log.md`. **Blocked by Phase 4's Pi deployment/validation** (backup timer + log must be live per `docs/progress-log.md`) — this task cannot be completed locally. **Status 2026-07-27:** the equivalent checklist is now published in `README.md` ("End-to-end validation checklist") ready for the operator; Part A was executed locally instead and passed.
